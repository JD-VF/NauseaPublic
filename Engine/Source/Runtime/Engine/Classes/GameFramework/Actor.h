...
//#NAUSEA_BEGIN_TAKE_DAMAGE_EVENT Multicast delegates do not properly pass FDamageEvent children, so we use this event instead for that and to be able to mutate incoming damage.
	/** Called when the actor is damaged. Can mutate damage amount. */
	DECLARE_EVENT_FiveParams(AActor, FTakeDamageEvent, AActor*, float&, const struct FDamageEvent&, class AController*, AActor*)
	FTakeDamageEvent OnTakeDamage;
//#NAUSEA_END_TAKE_DAMAGE_EVENT
