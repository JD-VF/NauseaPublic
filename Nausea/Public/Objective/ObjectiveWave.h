...
public:
	...
	static UObjectiveWave* CreateWaveObjective(const UObject* WorldContextObject, TSubclassOf<UObjectiveWave> ObjectiveClass, const FString& InObjectiveID, TSubclassOf<UObjectiveWaveConfig> WaveConfigOverrideClass, TArray<TScriptInterface<IWaveSpawnLocationInterface>> SpawnLocationList);

	template<class T>
	static T* CreateWaveObjective(const UObject* WorldContextObject, TSubclassOf<UObjectiveWave> ObjectiveClass, const FString& InObjectiveID, TSubclassOf<UObjectiveWaveConfig> WaveConfigOverrideClass, TArray<TScriptInterface<IWaveSpawnLocationInterface>> SpawnLocationList)
	{
	...
	//
	UFUNCTION()
	TArray<ACoreCharacter*> PerformSpawn(TArray<TSubclassOf<ACoreCharacter>>& SpawnRequest);
	UFUNCTION()
	...
	UFUNCTION(BlueprintCallable, Category = Objective)
	int32 GetRemainingWaveCount() const { return FMath::RoundToInt(float(TotalWaveSizeCount) * GetObjecitveProgress()); }
